...
const GetPlaylists = (props: Props) => {
  ...
  useEffect(() => {
    ...
    if (next > myPlaylists?.length && myPlaylists?.length !== totalPlaylists) {
  ...
  const getMorePlaylists = () => {
    // setOptions({ limit: 50, offset: +50 });
    const nextSetOfPlaylists = { limit: 50, offset: 50 };
    if (totalPlaylists !== myPlaylists?.length) {
      dispatch(getUserPlaylists(nextSetOfPlaylists));
    }
  };
  console.log(options);
    ...
    <>
      ...
      <div className="spotify__grid--loadmore">
        ...
        {showGetMorePlaylistsButton && (
          <button id="getMoreSpotifyPlaylistsButton" onClick={getMorePlaylists}>
            Load more
          </button>
        )}
      </div>
